...
module ServerLib {
   ...
   /*async*/ exposed function disconnect() {
    ...
    function read_content(f) {
        View.content x = 
            ...
            case {disconnected}: {welcome: <h2>Welcome to <b>Statbox</b>. Sign in with your <a href="www.dropbox.com">Dropbox</a> account to see your stats.</h2>}
            }
        f(x)
    } //FIXME: move the sentences somewhere else

    /*async*/ exposed function push_content(){
        read_content(View.set_content(_));
    }

    function read_data(path, f) {
        match (DropboxSession.get_uid()) {
        case {some:uid}:
            View.folder_info info =
    ...
    /*async*/ exposed function push_data(path){
        read_data(path, View.set_data(path, _));
    }
    ...
    /*async*/ exposed function move_to_path(string path) {
        ...
        read_data(path, View.set_data(path, _)); // N.B. we don't user push... to ensure that get_data is computed before the last call
        push_content()
    }
